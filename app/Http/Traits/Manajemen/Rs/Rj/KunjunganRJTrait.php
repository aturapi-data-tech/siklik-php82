 ...
 *
 * Konvensi (klinik pratama — hanya Rawat Jalan):
 *   - rj_status='I' dipetakan sebagai "Transfer UGD" (transfer biaya RJ ke UGD; lihat kasir-rj).
 *   - Pasien Baru = pass_status='N', Pasien Lama = pass_status='O' atau NULL.
 *   - BPJS = klaim_status='BPJS' OR klaim_id='JM' (JKN Mobile); selain itu = UMUM.
 *
 * Catatan: siklik (klinik pratama) tidak memakai klaim_id='KR' (Kronis) — itu konsep RS,
 * sudah dihapus dari modul RJ, jadi tidak ada filter exclude KR di sini.
 */
trait KunjunganRJTrait
{
    ...
    /**
     * Pasien unik global di rentang periode (distinct reg_no).
     */
