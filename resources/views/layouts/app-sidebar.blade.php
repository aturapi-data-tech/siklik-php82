...
<aside x-cloak id="app-sidebar"
    ...
    {{-- Super grafis: watermark logogram besar (sangat tipis) --}}
        ...
        <div class="flex items-center gap-2.5">
            ...
            <div class="leading-tight">
                @auth
                    ...
                    <div class="text-sm text-muted dark:text-gray-400">{{ auth()->user()->getRoleNames()->first() ?? 'User' }}</div>
                @else
    ...
    {{-- Garis aksen header — solid lime pendek --}}
    ...
    @php
        $defaultOpenKey = null;
        if (($sidebarMenus ?? collect())->isNotEmpty()) {
                ...
                <div x-cloak x-show="openMenus['{{ $key }}'] || menuQuery.length > 0" x-collapse class="mt-0.5 space-y-0.5 pl-3">
                    @foreach ($items as $item)
                        ...
                        <a href="{{ $item['href'] }}" wire:navigate
                            ...
                            class="block px-3 py-2.5 rounded-lg transition-colors duration-150 group/item
                                   {{ $isActiveItem ? 'bg-brand-green shadow-sm' : 'hover:bg-brand-green/10 dark:hover:bg-brand-lime/10' }}">
                            {{-- Judul --}}
                            <div class="text-base leading-snug {{ $isActiveItem ? 'font-bold text-white' : 'font-medium text-body group-hover/item:text-brand-green dark:text-gray-300 dark:group-hover/item:text-brand-lime' }}">
                                {{ $item['title'] }}
                            </div>
                            {{-- Deskripsi --}}
                            @if (!empty($item['desc']))
