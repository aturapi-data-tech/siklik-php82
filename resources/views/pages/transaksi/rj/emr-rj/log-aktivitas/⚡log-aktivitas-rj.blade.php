...
// Membaca jejak yg sudah direkam appendAdminLogRJ() → AdministrasiRJ.userLogs.
...
new class extends Component {
    ...
    public function open(int $rjNo): void
    {
        ...
        $this->dispatch('open-modal', name: 'log-aktivitas-rj');
    }
    ...
    /**
     * Tutup lewat server supaya rjNo dikosongkan: isi modal dibungkus guard if($rjNo),
     * jadi display pasien & daftar log ikut dihapus saat tutup, bukan tinggal tersembunyi.
     */
    ...
    private function loadLogs(int $rjNo): void
    {
        ...
        // Jejak tersebar di 2 tempat di siklik: root `userLogs` (legacy/pendaftaran —
        // "Form Entry Daftar RJ", tanpa userLogCat) + `AdministrasiRJ.userLogs`
        // (dari appendAdminLogRJ, dgn userLogCat ADMIN/MR). Gabung semua.
        $logs = array_merge(
            $data['userLogs'] ?? [],
            ...
            <div class="relative px-6 py-5 border-b border-gray-200 dark:border-gray-700">
                <div class="relative flex items-start justify-between gap-4">
                    <div class="flex-1 min-w-0">
                        <livewire:pages::transaksi.rj.display-pasien-rj.display-pasien-rj :rjNo="$rjNo" :showTtv="false"
                            ...
                            @foreach ($tabs as $val => $label)
                                <button type="button" wire:click="setFilter('{{ $val }}')"
                                    ...
                                    {{ $filterCat === $val
                                        ...
                                        : 'bg-gray-50 text-gray-500 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                    {{ $label }}
                                </button>
                            @endforeach
                        </div>
