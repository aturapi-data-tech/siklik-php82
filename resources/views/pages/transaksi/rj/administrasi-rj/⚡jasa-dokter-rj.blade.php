...
new class extends Component {
    ...
    public function removeJasaDokter(int $rjaccdocDtl): void
    {
        ...
        $itemDesc = collect($this->dataDaftarPoliRJ['JasaDokter'] ?? [])->firstWhere('rjaccdocDtl', $rjaccdocDtl)['JasaDokterDesc'] ?? '-';
                ...
                <tbody>
                    @forelse ($dataDaftarPoliRJ['JasaDokter'] ?? [] as $item)
                        <tr class="transition group hover:bg-gray-50 dark:hover:bg-gray-800/40">
                            ...
                            @if (!$isFormLocked)
                                <td class="ds-c">
                                    <x-hapus-button :action="'removeJasaDokter(' . $item['rjaccdocDtl'] . ')'"
                                        title="Hapus Jasa Dokter" message="Hapus jasa dokter ini?" />
                                </td>
                            @endif
                        </tr>
                    @empty
