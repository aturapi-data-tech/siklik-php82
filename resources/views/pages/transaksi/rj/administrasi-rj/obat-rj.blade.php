    ...
    <div class="p-4 border border-gray-200 rounded-2xl dark:border-gray-700 bg-gray-50 dark:bg-gray-800/40" x-data
        ...
        <div class="flex items-end gap-3 mb-3">
            ...
            <div class="w-20">
                ...
                <x-text-input wire:model="formEntryObat.qty" placeholder="Qty" class="w-full text-sm" x-ref="inputQty"
        ...
        <div class="flex items-end gap-3">
            ...
            <div class="w-32">
                ...
                <x-text-input wire:model="formEntryObat.ket" placeholder="Ket." class="w-full text-sm" x-ref="inputKet"
                    x-on:keyup.enter="$nextTick(() => $refs.inputExpDate?.focus())" />
            </div>
            <div class="w-36">
                <x-input-label value="Exp. Date" class="mb-1" />
                <x-text-input type="date" wire:model="formEntryObat.expDate" class="w-full text-sm" x-ref="inputExpDate"
            ...
            <div class="flex-1">
                ...
                <x-text-input wire:model="formEntryObat.catatanKhusus" placeholder="Catatan..." class="w-full text-sm"
                    x-ref="inputCatatan" x-on:keyup.enter="$wire.insertObat()" />
            </div>
            <div class="w-24">
            ...
            <div class="flex gap-2 pb-0.5">
                <button type="button" wire:click.prevent="insertObat" wire:loading.attr="disabled"
                    wire:target="insertObat" class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold
                    ...
                    <span wire:loading wire:target="insertObat">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    ...
                    <tr wire:key="obat-row-{{ $item['rjobatDtl'] }}-{{ $isEditing ? 'edit' : 'view' }}" x-data
                        ...
                        <td class="px-3 py-2 whitespace-nowrap">
                            @if ($isEditing)
                            <x-text-input wire:model="editRow.qty" class="w-16 text-sm text-right" x-ref="editQty"
                                x-init="$el.focus();
                                        $el.select()"
                            ...
                            @enderror
                            @else
                            <span class="block text-right text-gray-700 dark:text-gray-300">{{
                                number_format($item['qty']) }}</span>
                            @endif
                        </td>
                        {{-- Signa --}}
                        <td class="px-3 py-2 whitespace-nowrap">
                            ...
                            <div class="flex items-center gap-1">
                                ...
                                <x-text-input wire:model="editRow.kapsul" class="w-12 text-sm text-center"
                                    x-ref="editKapsul" x-on:keyup.enter="$nextTick(() => $refs.editTakar?.focus())" />
                            </div>
                            @else
                            <span class="block text-center text-gray-700 dark:text-gray-300">{{ $item['carapakai'] }}x{{
                                $item['kapsul'] }}</span>
                            @endif
                        </td>
                        {{-- Takar --}}
                        ...
                        <td class="px-3 py-2 whitespace-nowrap">
                            ...
                            <x-text-input wire:model="editRow.ket" placeholder="Ket." class="text-sm w-28"
                                x-ref="editKet" x-on:keyup.enter="$nextTick(() => $refs.editExpDate?.focus())" />
                            @else
                            <span class="text-xs text-gray-500 dark:text-gray-400">{{ $item['ket'] ?? '-' }}</span>
                            @endif
                        </td>
                        {{-- Exp Date --}}
                        <td class="px-3 py-2 whitespace-nowrap">
                            ...
                            <x-text-input type="date" wire:model="editRow.expDate" class="text-sm w-36"
                                x-ref="editExpDate" x-on:keyup.enter="$nextTick(() => $refs.editCatatan?.focus())" />
                            @error('editRow.expDate')
                            <p class="text-xs text-red-500 mt-0.5">{{ $message }}</p>
                            @enderror
                            @else
                            <span class="text-xs text-gray-500 dark:text-gray-400">{{ $item['expDate'] ?? '-' }}</span>
                            @endif
                        </td>
                        {{-- Catatan --}}
                        <td class="px-3 py-2 whitespace-nowrap">
                            ...
                            <x-text-input wire:model="editRow.catatanKhusus" placeholder="Catatan..."
                                class="text-sm w-36" x-ref="editCatatan" x-on:keyup.enter="$wire.saveEdit()" />
                            @else
                            <span class="text-xs text-gray-500 dark:text-gray-400">{{ $item['catatanKhusus'] ?? '-'
                                }}</span>
                            @endif
                        </td>
                        {{-- Etiket --}}
                        <td class="px-3 py-2 text-center whitespace-nowrap">
                            <x-ghost-button wire:click="cetakEtiketItem({{ $item['rjobatDtl'] }})"
                                wire:loading.attr="disabled" wire:target="cetakEtiketItem({{ $item['rjobatDtl'] }})"
                        ...
                        <td
                            ...
                            Rp
                            {{ number_format($isEditing ? $item['price'] * ($editRow['qty'] ?? $item['qty']) :
                            $item['total']) }}
                        </td>
                        {{-- Aksi --}}
                        @if (!$isFormLocked)
                        <td class="px-3 py-2 whitespace-nowrap">
                            @if ($isEditing)
                            <div class="flex items-center gap-1">
                                <x-secondary-button type="button" wire:click="saveEdit" wire:loading.attr="disabled"
                            ...
                            @else
                            <div class="flex items-center gap-1">
                                <x-secondary-button type="button" wire:click="startEdit({{ $item['rjobatDtl'] }})"
