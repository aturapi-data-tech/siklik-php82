...
/**
 * Pembayaran Hutang Non-Medis (Supplier barang ATK/RT).
 *
 * Port skema klinik dari modul Pembayaran Hutang PBF, tapi memakai
 * tabel/sequence *NONS sendiri agar tidak tabrakan dgn hutang medis
 * (rcv_no medis & non-medis bisa bertabrakan).
 *
 * Equivalent dgn form Oracle Forms angsuran + procedure post_transaksi_angsuran:
 *   - Pilih supplier
 ...
 *       * 1x INSERT SKTXN_CASHOUTHDRNONS (master cashout)
 *       * Per rcv yg dicentang (urut vcount):
 *           - INSERT SKTXN_RCVPAYMENTNONS
 *           - INSERT SKTXN_CASHOUTDTLNONS
 *           - UPDATE SKTXN_RCVHDRNONS rcv_status='L' (kalau lunas penuh)
 */
...
new class extends Component {
    ...
    /* ── LOV Listener ── */
    #[On('lov.selected.hutang-non-supplier')]
    ...
    public function rcvList()
    {
        ...
        $headers = DB::table('sktxn_rcvhdrnons')
            ->select([
                'rcv_no',
                'supp_id',
        ...
        $totalDetailMap = DB::table('sktxn_rcvdtlnons')
        ...
        // Total titipan (rcvpaymentnons) per rcv
        $titipanMap = DB::table('sktxn_rcvpaymentnons')
    ...
    public function toggleCheckBox(int $rcvNo): void
    {
        ...
        if (!$isCurrentlyChecked) {
            ...
            $nextVcount = (int) DB::table('sktxn_rcvhdrnons')
                ->where('supp_id', $this->suppId)
    ...
    public function toggleCheckBoxAll(): void
    {
        ...
        if ($allChecked) {
            ...
            $rcvs = DB::table('sktxn_rcvhdrnons')
                ->select('rcv_no')
                ->where('supp_id', $this->suppId)
                ->where('rcv_status', 'H')
                ->orderBy('rcv_date')
    ...
    public function openProsesBayar(): void
    {
        ...
        $this->dispatch('hutang-non.openProsesBayar',
            suppId: $this->suppId,
...
<div>
    <x-page-title
        ...
        subtitle="Pelunasan / angsuran hutang ke supplier barang non-medis (ATK/RT)" />
            ...
            <div class="bg-white border border-gray-200 shadow-sm rounded-2xl dark:border-gray-700 dark:bg-gray-900">
                ...
                <div class="px-5 py-5">
                    <livewire:lov.supplier.lov-supplier
                        target="hutang-non-supplier"
                        label="Cari Supplier"
                        placeholder="Ketik kode / nama / telp supplier..."
                        :initialSuppId="$suppId"
                        wire:key="lov-supplier-hutang-non-{{ $suppId ?? 'empty' }}" />
                </div>
            </div>
                        ...
                        <table class="ds-table">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-right text-emerald-700">Titipan</th>
                                    <th class="text-right text-rose-700">Sisa</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse($this->rcvList as $row)
                                    <tr wire:key="hutang-non-{{ $row->rcv_no }}"
                                        ...
                                        <td class="whitespace-nowrap">{{ $row->rcv_date_display }}</td>
                                        ...
                                        <td class="ds-td-token text-right text-emerald-700">{{ number_format($row->titipan) }}</td>
                            ...
                            @if ($this->rcvList->isNotEmpty())
                                <tfoot class="text-gray-700 bg-gray-50 dark:bg-gray-800 dark:text-gray-200">
                                    <tr class="font-semibold">
                                        ...
                                        <td class="ds-td-token text-right text-emerald-700">Rp {{ number_format($this->summary['tottitipan']) }}</td>
