...
new class extends Component {
    ...
    // ── Detail entry (input barang) — NON-MEDIS: TANPA batch & exp_date ──
    ...
    #[On('lov.selected.product-non-rcv')]
    public function onProductSelected(string $target, ?array $payload): void
    {
        // Payload kontrak LOV product-non: {product_id, product_name, cost_price, qty_box}
    ...
    /* ══════════════════════════════
     ...
     | NON-MEDIS: tanpa validasi batch / exp_date.
     ══════════════════════════════ */
    ...
    /* ══════════════════════════════
     | LOAD DETAILS FROM DB (EDIT) — tanpa batch/ED (kolom tdk ada di dtlnons)
     ══════════════════════════════ */
    ...
    /* ══════════════════════════════
     ...
     |   - edit    : decrement qty lama (sebelum delete dtl lama), lalu increment qty baru
     ══════════════════════════════ */
    public function simpan(): void
    {
        ...
        try {
            ...
            $msg = $rcvStatus === 'L' ? 'Barang non-medis — lunas, disimpan & diposting.' : 'Barang non-medis — status Hutang, disimpan & diposting.';
            $this->dispatch('toast', type: 'success', message: $msg);
            $this->closeBayar();
            $this->closeModal();
            $this->dispatch('penerimaan-non.saved');
        } catch (\Exception $e) {
    ...
    /* ══════════════════════════════
     | DELETE
     | STOK: decrement balik qty_box per item sebelum hapus dtl (no legacy trigger).
     ══════════════════════════════ */
    ...
    /* ══════════════════════════════
     | BATAL TRANSAKSI
     |   - status 'A'      → set 'F' + decrement balik qty_box (barang dianulir)
     |   - status 'H'/'L'  → rollback cashout+payment + set status kembali 'A'
     |                       (qty_box TIDAK diubah: barang tetap diterima, hanya
     |                        pembayaran yang dirollback ke Daftar Tunggu).
     ══════════════════════════════ */
    ...
    public function closeBayar(): void
    {
        ...
        $this->resetErrorBag('cbId');
        $this->hitungSemua();
        // Bump versi area 'bayar' supaya LOV cara-bayar remount (state bersih).
        $this->incrementVersion('bayar');
    ...
    {
        $this->reset(['rcvNo', 'rcvDate', 'dueDate', 'suppId', 'suppName', 'rcvDesc', 'rcvStatus', 'entryProductId', 'entryProductName', 'entryQty', 'entryCostPrice', 'entryDiscount1', 'entryDiscount2', 'details', 'totalBarang', 'totalQty', 'rcvDiskon', 'totalSetelahDiskon', 'rcvPpn', 'rcvPpnStatus', 'ppnNominal', 'rcvMaterai', 'grandTotal', 'bayar', 'sisa', 'pendingPriceUpdate', 'cbId', 'cbDesc']);
        $this->detailCounter = 0;
    ...
    {
        $this->reset(['entryProductId', 'entryProductName', 'entryQty', 'entryCostPrice', 'entryDiscount1', 'entryDiscount2']);
    }
};
?>
                            ...
                            @php
                                [$modalTitle, $modalSubtitle, $modeBadgeLabel, $modeBadgeVariant] = match (true) {
                                    $formMode === 'create' => [
                                        ...
                                        'Catat penerimaan barang non-medis (ATK/RT) baru dari Supplier.',
                                        'Tambah Baru',
                                        'success',
                                    ],
                                    in_array($rcvStatus, ['H', 'L'], true) => [
                                        "Lihat Barang Non-Medis #{$rcvNo}",
                                    ...
                                    default => [
                                        "Edit Barang Non-Medis #{$rcvNo}",
                                        'Ubah data penerimaan barang non-medis (Daftar Tunggu).',
                                        'Edit',
                                        'warning',
                                    ],
                                };
                    ...
                    @else
                        <x-border-form :title="($isReadOnly ? 'Keranjang (' : 'Tambah Barang / Keranjang (') .
                            ...
                            @if (!$isReadOnly)
                                <div class="p-4 ">
                                    <div
                                        class="grid grid-cols-2 gap-3 sm:grid-cols-[2.5fr_0.7fr_repeat(3,1fr)_auto] items-start">
                                        {{-- Barang --}}
                                        <div class="col-span-2 sm:col-span-1" x-ref="entryProductWrapper">
                                            <div>
                                                <livewire:lov.product-non.lov-product-non target="product-non-rcv"
                                                    label="Barang" :initialProductId="$entryProductId"
                                        ...
                                        {{-- Disc 2 (NON-MEDIS: enter langsung ke tambahBarang — tanpa batch/ED) --}}
                                        <div>
                                            <div>
                            ...
                            {{-- ═══ TABEL KERANJANG — lanjutan section yang sama (NON-MEDIS: tanpa kolom Batch/ED) ═══ --}}
