...
//
// Adaptasi siklik:
//  - users siklik PUNYA myuser_code / myuser_name; TIDAK punya myuser_profesi / emp_id
//    (emp_id sudah di-rename jadi kasir_id). Profesi diganti ROLE pertama user (spatie).
//  - Role gate ganti ke 'admin' (role siklik lowercase).
//  - Kolom last_seen_at / last_seen_route baru aktif setelah alter_users_add_last_seen.sql
//    dideploy — kalau semua NULL, UI kasih baris info.
...
new class extends Component {
    ...
    // True kalau di tabel users belum ada satupun last_seen_at terisi
    // (kemungkinan alter SQL belum dideploy / belum ada aktivitas).
    public bool $featureDormant = false;
    ...
    public function refresh(): void
    {
        ...
        $this->rows = $userRows->map(function ($row) use ($rolesByUserId, $routeLabelMap) {
            ...
            return [
                ...
                'email' => $row->email,
                // Profesi siklik = role pertama user (siklik tidak punya kolom profesi).
                'profesi' => !empty($userRoles) ? $userRoles[0] : '-',
        ...
        // Deteksi fitur dormant: tidak ada satupun user dengan last_seen_at terisi
        // (hanya cek kalau hasil kosong, biar tidak query mahal saat sudah ada data).
        if ($this->totalOnline === 0) {
            $anySeen = DB::table('users')->whereNotNull('last_seen_at')->limit(1)->count();
            $this->featureDormant = $anySeen === 0;
        } else {
            $this->featureDormant = false;
        }
    }
};
?>
...
<div>
    ...
    <div class="w-full h-[calc(100vh-5rem)] flex flex-col bg-white dark:bg-gray-800">
        <div class="flex flex-col flex-1 min-h-0 px-6 pt-2 pb-6">
            ...
            {{-- INFO: fitur baru aktif setelah alter SQL dideploy --}}
            @if ($featureDormant)
                <div class="mt-3 px-4 py-3 text-sm rounded-lg bg-amber-50 border border-amber-200 text-amber-800 dark:bg-amber-900/20 dark:border-amber-900/50 dark:text-amber-300">
                    <b>Belum ada data aktivitas.</b> Kolom <code>last_seen_at</code> belum terisi untuk user manapun.
                    Pastikan SQL <code>alter_users_add_last_seen.sql</code> sudah dideploy dan middleware
                    <code>TrackUserActivity</code> aktif — data akan muncul setelah user melakukan request berikutnya.
                </div>
            @endif
