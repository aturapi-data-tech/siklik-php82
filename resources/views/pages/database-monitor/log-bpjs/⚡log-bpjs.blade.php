<?php
// resources/views/pages/database-monitor/log-bpjs/log-bpjs.blade.php
// Log BPJS API — riwayat pemanggilan API BPJS (tabel web_log_status).
//
// Adaptasi siklik (klinik pratama — hanya PCare, Antrean FKTP, i-Care):
//  - web_log_status di siklik ditulis oleh trait PcareTrait / AntrianTrait /
//    TraitJWTAntreanFktp / iCareTrait. Kolom yang TERISI: code, date_ref,
//    response, http_req, requestTransferTime. Kolom http_payload TIDAK PERNAH
//    diisi trait siklik → query tidak ambil kolom itu, panel Request Body
//    selalu tampil catatan "tidak ada request body".
//  - Kategori service derive dari pola http_req trait siklik:
//      PCare        → URL mengandung 'pcare'
//      i-Care       → URL mengandung 'icare' atau '/api/rs/validate'
//      Antrean FKTP → 'antreanrs' / '/jadwaldokter/' / '/antrean' / '/api/auth'
//                     / '/api/peserta' / '/api/ref/dokter' (outbound RS + inbound FKTP)
//      Lainnya      → selain itu
//    Tidak ada VClaim/Aplicares/SIRS/iDRG di klinik pratama.
//  - Role gate 'admin' (role siklik lowercase).
...
new class extends Component {
    ...
    public string $service = 'all'; // all | pcare | antrean | icare | other

    #[Url(as: 'st', history: true)]
    public string $statusFilter = 'all'; // all | 2xx | 4xx | 5xx | spesifik (200, 404, ...)
    ...
    // Pola URL per kategori (lowercase) — sumber tunggal untuk filter & deteksi.
    private const SERVICE_PATTERNS = [
        'pcare'   => ['pcare'],
        'antrean' => ['antreanrs', '/jadwaldokter/', '/antrean', '/api/auth', '/api/peserta', '/api/ref/dokter'],
        'icare'   => ['icare', '/api/rs/validate'],
    ];

    public function mount(): void
    {
        abort_unless(auth()->check() && auth()->user()->hasAnyRole(['Admin']), 403);
    ...
    /**
     * Deteksi service dari URL (pola siklik: PCare / Antrean FKTP / i-Care / Lainnya).
     */
    public static function detectService(?string $url): string
    {
        if (empty($url)) return 'other';
        $u = strtolower($url);
        foreach (self::SERVICE_PATTERNS as $svc => $patterns) {
            foreach ($patterns as $p) {
                if (str_contains($u, $p)) return $svc;
            }
        }
        return 'other';
    }
    ...
    {
        // Catatan: http_payload TIDAK diquery — trait siklik tidak pernah mengisinya.
        ...
        if ($this->service !== 'all') {
            if ($this->service === 'other') {
                // Lainnya = tidak match satupun pola service yang dikenal.
                $allPatterns = array_merge(...array_values(self::SERVICE_PATTERNS));
                $q->where(function ($sub) use ($allPatterns) {
                    foreach ($allPatterns as $p) {
                        $sub->whereRaw('LOWER(http_req) NOT LIKE ?', ['%' . strtolower($p) . '%']);
                    }
                });
            } else {
                $patterns = self::SERVICE_PATTERNS[$this->service] ?? null;
                if ($patterns) {
...
<div>
    <x-page-title
        title="Log BPJS API"
        subtitle="Riwayat pemanggilan PCare, Antrean FKTP & i-Care" />
                    ...
                    <div class="w-full sm:flex-1">
                        ...
                        <div class="relative mt-1">
                            ...
                            <x-text-input wire:model.live.debounce.400ms="search" type="search"
                                ...
                                placeholder="Cari URL endpoint / response (noKartu, pcare, error...)" />
                        </div>
                    </div>
                    ...
                    <table class="w-full min-w-full text-sm border-separate border-spacing-y-2 table-fixed">
                        ...
                        @forelse ($rows as $r)
                            @php
                                ...
                                $svcStyle = match ($svc) {
                                    'pcare'   => 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300',
                                    'antrean' => 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300',
                                        ...
                                        <div
                                            ...
                                            <div
                                                ...
                                                <div class="p-3 space-y-2">
                                                    ...
                                                    {{-- Request Body — siklik tidak menyimpan http_payload --}}
                                                    <div
                                                        class="pt-2 mt-1 border-t border-blue-100 text-[11px] italic text-gray-400 dark:border-blue-900/30 dark:text-gray-500">
                                                        (request body tidak disimpan — log siklik hanya menyimpan URL & response)
                                                    </div>
                                                </div>
                                            </div>
